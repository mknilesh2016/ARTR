...
// Name		:   Nilesh Mahajan
// Roll No.	:   ARTR01-109
// Program	:   05-Two2DShapes/01-BW/02-MultiplePipelines
// 
// ************************************************************************* //
...
VkResult Initialize(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("Initialize: CreatePipeline() failed with %i for triangle.", vkResult);
        return (vkResult);
    }
    else
    {
        LOGF("Initialize: CreatePipeline() succeded for triangle.");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("Initialize: CreatePipeline() failed with %i for rectangle.", vkResult);
        return (vkResult);
    }
    else
    {
        LOGF("Initialize: CreatePipeline() succeded for rectangle.");
    }
...
VkResult Resize(int width, int height)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("Resize: CreatePipeline() failed with %i for triangle.", vkResult);
        return (vkResult);
    }
    // Create Pipeline for rectangle
    vkResult = CreatePipeline(&vkPipeline_rectangle, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
    if (vkResult != VK_SUCCESS)
    {
        LOGF("Resize: CreatePipeline() failed with %i for rectangle.", vkResult);
        return (vkResult);
    }
    // Create Framebuffers
    vkResult = CreateFramebuffers();
...
void Uninitialize(void)
{
    ...
    if (vkDevice != VK_NULL_HANDLE)
    {
        ...
        if (vkPipeline_triangle != VK_NULL_HANDLE)
        {
            LOGF("Uninitialize: Destroying vkPipeline_triangle");
            vkDestroyPipeline(vkDevice, vkPipeline_triangle, NULL);
            vkPipeline_triangle = VK_NULL_HANDLE;
            LOGF("Uninitialize: vkPipeline_triangle destroyed");
        }

        if (vkPipeline_rectangle != VK_NULL_HANDLE)
        {
            LOGF("Uninitialize: Destroying vkPipeline_rectangle");
            vkDestroyPipeline(vkDevice, vkPipeline_rectangle, NULL);
            vkPipeline_rectangle = VK_NULL_HANDLE;
            LOGF("Uninitialize: vkPipeline_rectangle destroyed");
        }
...
VkResult CreatePipeline(VkPipeline* pPipeline, VkPrimitiveTopology topology)
{
    ...
    LOGF("CreatePipeline: Returned pipeline");
