...
// Name		:   Nilesh Mahajan
// Roll No.	:   ARTR01-109
// Program	:   03-Triangle/22b-StagingVertexBuffer
// 
// ************************************************************************* //
...
VkResult CreateVertexBuffer(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkCreateBuffer for staging vertex buffer failed with %d", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkCreateBuffer for staging vertex buffer succeded");
    }

    // Get device memory requirements for the above buffer
    VkMemoryRequirements vkMemoryRequirements_stagingBuffer;
    memset(&vkMemoryRequirements_stagingBuffer, 0, sizeof(VkMemoryRequirements));
    vkGetBufferMemoryRequirements(
        vkDevice,
        vertexData_stagingBuffer_position.vkBuffer,
        &vkMemoryRequirements_stagingBuffer);

    // Now allocate memory for the staging vertex buffer
    VkMemoryAllocateInfo vkMemoryAllocateInfo_stagingBuffer;
    memset(&vkMemoryAllocateInfo_stagingBuffer, 0, sizeof(VkMemoryAllocateInfo));
    ...
    {
        LOGF("CreateVertexBuffer: unable to locate required memory type for staging vertex buffer");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    else
    {
        LOGF("CreateVertexBuffer: Found equired memory type for staging vertex buffer at index %d", vkMemoryAllocateInfo_stagingBuffer.memoryTypeIndex);
    }

    // Allocate memory and get its handle
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkAllocateMemory for staging vertex buffer failed with %d", vkResult);

        if (vertexData_stagingBuffer_position.vkBuffer != VK_NULL_HANDLE)
        {
            LOGF("CreateVertexBuffer: Freeing staging vkBuffer");
            vkDestroyBuffer(vkDevice, vertexData_stagingBuffer_position.vkBuffer, NULL);
            LOGF("CreateVertexBuffer: Freed staging vkBuffer");
    ...
    LOGF("CreateVertexBuffer: vkAllocateMemory for staging vertex buffer succeded");
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkBindBufferMemory for staging vertex buffer failed with %d", vkResult);

        if (vertexData_stagingBuffer_position.vkDeviceMemory != VK_NULL_HANDLE)
        {
            LOGF("CreateVertexBuffer: Freeing staging buffer device memory");
            vkFreeMemory(vkDevice, vertexData_stagingBuffer_position.vkDeviceMemory, NULL);
            LOGF("CreateVertexBuffer: Freed staging buffer device memory");
    ...
    LOGF("CreateVertexBuffer: vkBindBufferMemory for staging vertex buffer succeded");
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkMapMemory for staging vertex buffer failed with %d", vkResult);
    ...
    {
        LOGF("CreateVertexBuffer: vkMapMemory for staging vertex buffer succeded");
    }

    // Copy vertex data to the mapped allocation
    LOGF("CreateVertexBuffer: writing vertex data to mapped staging vertex memory");
    memcpy(data, triangle_position, sizeof(triangle_position));
    LOGF("CreateVertexBuffer: wrote vertex data to mapped staging vertex memory");

    // Unmap memory
    LOGF("CreateVertexBuffer: unmapping staging vertex device memory");
    ...
    // No need for vkMapMemory, memcpy, vkUnmapMemory for device local buffer since CPU based writing is not involved.

    // Now create special buffer for staging copy
    VkCommandBufferAllocateInfo vkCommandBufferAllocateInfo;
    memset(&vkCommandBufferAllocateInfo, 0, sizeof(VkCommandBufferAllocateInfo));
    vkCommandBufferAllocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    vkCommandBufferAllocateInfo.pNext = NULL;
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkAllocateCommandBuffers for staging copy failed with %d", vkResult);
    ...
    LOGF("CreateVertexBuffer: vkAllocateCommandBuffers for staging copy succeded");
    ...
    vkCommandBufferBeginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT; // Due to this, vkResetCommandBuffer is not required
    vkResult = vkBeginCommandBuffer(vkCommandBuffer, &vkCommandBufferBeginInfo);
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkBeginCommandBuffer failed for staging copy command buffer. VkResult = %d", vkResult);
    ...
    LOGF("CreateVertexBuffer: vkBeginCommandBuffer for staging copy command buffer succeded.");

    // Queue buffer copy operation
    VkBufferCopy vkBufferCopy;
    memset(&vkBufferCopy, 0, sizeof(VkBufferCopy));
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkEndCommandBuffer failed for staging copy command buffer. VkResult = %d", vkResult);
    ...
    LOGF("CreateVertexBuffer: vkEndCommandBufferfor for staging copy command buffer succeded.");
    ...
    // No need to fill in synchronisation related members since the same is not required here.

    // Now submit above work to the queue
    vkResult = vkQueueSubmit(vkQueue, 1, &vkSubmitInfo, VK_NULL_HANDLE);
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkQueueSubmit() for staging copy command buffer failed with %i.", vkResult);
    ...
    LOGF("CreateVertexBuffer: vkQueueSubmit() for staging copy command buffer succeded. Waiting for the copy operation to complete");

    // Wait for the device to become idle
    vkResult = vkQueueWaitIdle(vkQueue);
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkQueueWaitIdle for staging copy command buffer failed with %d.", vkResult);
    ...
    LOGF("CreateVertexBuffer: Copy from staging buffer to device local vertex buffer is completed");

    // Free command buffer
    vkFreeCommandBuffers(vkDevice, vkCommandPool, 1, &vkCommandBuffer);

    LOGF("CreateVertexBuffer: Freed command buffer");
...
VkResult CreatePipeline(void)
{
    ...
    // Describes about shader attribute => layout(location = 0) in vec4 vPosition;
    ...
    vkPipelineRasterizationStateCreateInfo.cullMode = VK_CULL_MODE_BACK_BIT; // We can use back and front culling to save rateriser state in case of point and line drawing
    vkPipelineRasterizationStateCreateInfo.frontFace = VK_FRONT_FACE_CLOCKWISE; // front facing triangle orientation to be used for culling
