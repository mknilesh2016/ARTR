...
// Name		:   Nilesh Mahajan
// Roll No.	:   ARTR01-109
// Program	:   03-Triangle/34-ColoredTriangle
// 
// ************************************************************************* //
...
void Uninitialize(void)
{
    ...
    if (vkDevice != VK_NULL_HANDLE)
    {
        ...
        if (vertexData_color.vkBuffer != VK_NULL_HANDLE)
        {
            LOGF("Uninitialize: Freeing vkBuffer for vertexData_color");
            vkDestroyBuffer(vkDevice, vertexData_color.vkBuffer, NULL);
            LOGF("Uninitialize: Freed vkBuffer for vertexData_color");
            vertexData_color.vkBuffer = VK_NULL_HANDLE;
        }
        if (vertexData_color.vkDeviceMemory != VK_NULL_HANDLE)
        {
            LOGF("Uninitialize: Freeing device memory for vertexData_color");
            vkFreeMemory(vkDevice, vertexData_color.vkDeviceMemory, NULL);
            LOGF("Uninitialize: Freed device memory for vertexData_color");
...
VkResult CreateVertexBuffer(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkCreateBuffer for vertexData_position failed with %d", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkCreateBuffer for vertexData_position succeded");
    }
    ...
    {
        LOGF("CreateVertexBuffer: unable to locate required memory type for vertexData_position");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    else
    {
        LOGF("CreateVertexBuffer: Found equired memory type at index %d for vertexData_position", vkMemoryAllocateInfo.memoryTypeIndex);
    }

    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_position.vkDeviceMemory);
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkAllocateMemory failed with %d for vertexData_position", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkAllocateMemory succeded for vertexData_position");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkBindBufferMemory failed with %d for vertexData_position", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkBindBufferMemory succeded for vertexData_position");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkMapMemory failed with %d for vertexData_position", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkMapMemory succeded for vertexData_position");
    }

    // Copy vertex data to the mapped allocation
    LOGF("CreateVertexBuffer: writing vertex data to mapped memory for vertexData_position");
    memcpy(data, triangle_position, sizeof(triangle_position));
    LOGF("CreateVertexBuffer: wrote vertex data to mapped memory for vertexData_position");

    // Unmap memory
    LOGF("CreateVertexBuffer: unmapping vertex device memory for vertexData_position");
    vkUnmapMemory(vkDevice, vertexData_position.vkDeviceMemory);
    LOGF("CreateVertexBuffer: unmapped vertex device memory for vertexData_position");

    // ---------------------------------------------------- Vertex Color Buffer -------------------------------------------------------- //
    // Reset global variable
    memset(&vertexData_color, 0, sizeof(VertexData));
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkCreateBuffer for vertexData_color failed with %d", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkCreateBuffer for vertexData_color succeded");
    }
    ...
    if (foundMemoryTypeIndex == FALSE)
    {
        LOGF("CreateVertexBuffer: unable to locate required memory type for vertexData_color");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    else
    {
        LOGF("CreateVertexBuffer: Found equired memory type at index %d for vertexData_color", vkMemoryAllocateInfo.memoryTypeIndex);
    }

    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_color.vkDeviceMemory);
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkAllocateMemory failed with %d for vertexData_color", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkAllocateMemory succeded for vertexData_color");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkBindBufferMemory failed with %d for vertexData_color", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkBindBufferMemory succeded for vertexData_color");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkMapMemory failed with %d for vertexData_color", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkMapMemory succeded for vertexData_color");
    }

    // Copy vertex data to the mapped allocation
    LOGF("CreateVertexBuffer: writing vertex data to mapped memory for vertexData_color");
    memcpy(data, triangle_color, sizeof(triangle_color));
    LOGF("CreateVertexBuffer: wrote vertex data to mapped memory for vertexData_color");

    // Unmap memory
    LOGF("CreateVertexBuffer: unmapping vertex device memory for vertexData_color");
    vkUnmapMemory(vkDevice, vertexData_color.vkDeviceMemory);
    LOGF("CreateVertexBuffer: unmapped vertex device memory for vertexData_color");
