...
// Name		:   Nilesh Mahajan
// Roll No.	:   ARTR01-109
// Program	:   03-Triangle/33-IndexedDrawing
// 
// ************************************************************************* //
...
void Uninitialize(void)
{
    ...
    if (vkDevice != VK_NULL_HANDLE)
    {
        ...
        if (vertexData_position_index.vkBuffer != VK_NULL_HANDLE)
        {
            LOGF("Uninitialize: Freeing vkBuffer for vertexData_position_index");
            vkDestroyBuffer(vkDevice, vertexData_position_index.vkBuffer, NULL);
            LOGF("Uninitialize: Freed vkBuffer for vertexData_position_index");
            vertexData_position_index.vkBuffer = VK_NULL_HANDLE;
        }
        if (vertexData_position_index.vkDeviceMemory != VK_NULL_HANDLE)
        {
            LOGF("Uninitialize: Freeing device memory for vertexData_position_index");
            vkFreeMemory(vkDevice, vertexData_position_index.vkDeviceMemory, NULL);
            LOGF("Uninitialize: Freed device memory for vertexData_position_index");
...
VkResult CreateVertexBuffer(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkCreateBuffer failed for vertex position with %d", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkCreateBuffer for vertex position succeded");
    }
    ...
    {
        LOGF("CreateVertexBuffer: unable to locate required memory type for vertex position");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    else
    {
        LOGF("CreateVertexBuffer: Found required memory type at index %d for vertex position", vkMemoryAllocateInfo.memoryTypeIndex);
    }

    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_position.vkDeviceMemory);
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkAllocateMemory failed with %d for vertex position", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkAllocateMemory succeded for vertex position");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkBindBufferMemory failed with %d for vertex position", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkBindBufferMemory succeded for vertex position");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkMapMemory failed with %d for vertex position", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkMapMemory succeded for vertex position");
    }

    // Copy vertex data to the mapped allocation
    LOGF("CreateVertexBuffer: writing vertex position data to mapped memory");
    memcpy(data, triangle_position, sizeof(triangle_position));
    LOGF("CreateVertexBuffer: wrote vertex position data to mapped memory");

    // Unmap memory
    LOGF("CreateVertexBuffer: unmapping vertex position device memory");
    vkUnmapMemory(vkDevice, vertexData_position.vkDeviceMemory);
    LOGF("CreateVertexBuffer: unmapped vertex position device memory");
    // ========================================== Vertex Position Buffer ========================================================//

    // ========================================== Position Index Buffer ========================================================//
    // Reset global variable
    memset(&vertexData_position_index, 0, sizeof(VertexData));
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkCreateBuffer failed with %d for position index buffer", vkResult);
        return vkResult;
    }
    else
    {
    ...
    if (foundMemoryTypeIndex == FALSE)
    {
        LOGF("CreateVertexBuffer: unable to locate required memory type for position index buffer");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    else
    {
        LOGF("CreateVertexBuffer: Found equired memory type at index %d for position index buffer", vkMemoryAllocateInfo.memoryTypeIndex);
    }

    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vertexData_position_index.vkDeviceMemory);
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkAllocateMemory failed with %d for position index buffer", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkAllocateMemory succeded for position index buffer");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkBindBufferMemory failed with %d for position index buffer", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkBindBufferMemory succeded for position index buffer");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateVertexBuffer: vkMapMemory failed with %d for position index buffer", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateVertexBuffer: vkMapMemory succeded for position index buffer");
    }

    // Copy vertex data to the mapped allocation
    LOGF("CreateVertexBuffer: writing vertex position index data to mapped memory");
    memcpy(data, triangle_position_indices, sizeof(triangle_position_indices));
    LOGF("CreateVertexBuffer: wrote vertex position index data to mapped memory");

    // Unmap memory
    LOGF("CreateVertexBuffer: unmapping vertex position index data device memory");
    vkUnmapMemory(vkDevice, vertexData_position_index.vkDeviceMemory);
    LOGF("CreateVertexBuffer: unmapped vertex position index data device memory");
