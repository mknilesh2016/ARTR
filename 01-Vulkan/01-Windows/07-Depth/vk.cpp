...
// Color Images and Image Views related variables
uint32_t swapchainImageCount = UINT32_MAX;
VkImage* swapchainImage_array = NULL;
VkImageView* swapchainImageView_array = NULL;
// Depth image and image view related variables
...
VkResult Initialize(void)
{
    ...
    memset(&vkClearDepthStencilValue, 0, sizeof(VkClearDepthStencilValue));
...
void Uninitialize(void)
{
    ...
    if (vkDevice != VK_NULL_HANDLE)
    {
        ...
        if (vkImageView_depth != VK_NULL_HANDLE)
        {
            LOGF("Uninitialize: Destroying vkImageView_depth");
            vkDestroyImageView(vkDevice, vkImageView_depth, NULL);
            vkImageView_depth = VK_NULL_HANDLE;
            LOGF("Uninitialize: vkImageView_depth destroyed");
        }
        if (vkImage_depth != VK_NULL_HANDLE)
        {
            LOGF("Uninitialize: Destroying vkImage_depth");
            vkDestroyImage(vkDevice, vkImage_depth, NULL);
            vkImage_depth = VK_NULL_HANDLE;
            LOGF("Uninitialize: vkImage_depth destroyed");
        }
        if (vkDeviceMemory_depth != VK_NULL_HANDLE)
        {
            LOGF("Uninitialize: Destroying vkDeviceMemory_depth");
            vkFreeMemory(vkDevice, vkDeviceMemory_depth, NULL);
            vkDeviceMemory_depth = VK_NULL_HANDLE;
            LOGF("Uninitialize: vkDeviceMemory_depth destroyed");
        }
...
VkResult CreateImagesAndImageViews(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateImagesAndImageViews: GetSupportedDepthFormat() failed with %d", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateImagesAndImageViews: GetSupportedDepthFormat() succeded");
    }

    // For depth image, initialize VkImageCreateInfo structure
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateImagesAndImageViews: vkCreateImage failed with %d", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateImagesAndImageViews: vkCreateImage succeded");
    } 

    // Get device memory requirements for depth image
    ...
    {
        LOGF("CreateImagesAndImageViews: unable to locate required memory type");
        return VK_ERROR_INITIALIZATION_FAILED;
    }
    else
    {
        LOGF("CreateImagesAndImageViews: Found required memory type at index %d", vkMemoryAllocateInfo.memoryTypeIndex);
    }

    vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vkDeviceMemory_depth);
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateImagesAndImageViews: vkAllocateMemory failed with %d", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateImagesAndImageViews: vkAllocateMemory succeded");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateImagesAndImageViews: vkBindImageMemory failed with %d", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateImagesAndImageViews: vkBindImageMemory succeded");
    }
    ...
    if (vkResult != VK_SUCCESS)
    {
        LOGF("CreateImagesAndImageViews: vkCreateImageView failed with %d", vkResult);
        return vkResult;
    }
    else
    {
        LOGF("CreateImagesAndImageViews: vkCreateImageView succeded");
    }
...
VkResult GetSupportedDepthFormat(void)
{
    ...
    for (uint32_t i = 0; i < _ARRAYSIZE(vkFormat_depth_arr); ++i)
    {
        VkFormatProperties vkFormatProperties;
        memset(&vkFormatProperties, 0, sizeof(VkFormatProperties));
        vkGetPhysicalDeviceFormatProperties(vkPhysicalDevice_selected, vkFormat_depth_arr[i], &vkFormatProperties);
        if (vkFormatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT)
        {
            vkFormat_depth = vkFormat_depth_arr[i];
            break;
        }
    }
    ...
    {
        LOGF("GetSupportedDepthFormat: unable to locate supported depth format");
        return VK_ERROR_INITIALIZATION_FAILED;
    }

    LOGF("GetSupportedDepthFormat: Found supported depth format %d", vkFormat_depth);
...
VkResult CreateRenderPass(void)
{
    ...
    memset(&vkAttachmentReference_color, 0, sizeof(VkAttachmentReference));
    ...
    memset(&vkAttachmentReference_depth, 0, sizeof(VkAttachmentReference));
...
VkResult CreatePipeline(void)
{
    ...
    memset(&vkPipelineDepthStencilStateCreateInfo, 0, sizeof(VkPipelineDepthStencilStateCreateInfo));
...
VkResult CreateFramebuffers(void)
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; ++i)
    {
        VkImageView vkImageView_attachments_array[2]; // Color and depth
...
VkResult BuildCommandBuffers(void)
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; ++i)
    {
        ...
        // Set clear values
        VkClearValue vkClearValue_array[2]; // Color and depth
